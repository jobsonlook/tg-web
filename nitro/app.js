...
function initMapScreen() {
    ...
    levelTheme = "desert",
    levelNum = 8;
    for (var g = 0; g < aMapPointData.length; g++)
        if (2 == saveDataHandler.aLevelStore[3 * g]) {
            var h = {
    ...
    for (var g = 0; g < aPowerUpBarData.length; g++)
        aPowerUpBarData[g] = saveDataHandler.aLevelStore[27 + g];
...
function butEventHandler(a, b) {
    ...
    case "resetScores":
        ...
        break;
    case "steerLeft":
        b.isDown ? (leftSteer = 300 + speed,
        leftSteerSimple = 1,
        rightSteer = 0,
        rightSteerSimple = 0) : (leftSteer = 0,
        leftSteerSimple = 0);
        break;
    case "steerRight":
        b.isDown ? (rightSteer = -(300 + speed),
        rightSteerSimple = -1,
        leftSteer = 0,
        leftSteerSimple = 0) : (rightSteer = 0,
        rightSteerSimple = 0);
        break;
    ...
    case "upgradeScreen":
        ...
        break;
    case "powerUp0":
        winnings >= aPowerUpButsData[aPowerUpBarData[0]] && aPowerUpBarData[0] < 8 && (playSound("upgrade"),
        ...
        break;
    case "powerUp1":
        winnings >= aPowerUpButsData[aPowerUpBarData[1]] && aPowerUpBarData[1] < 8 && (playSound("upgrade"),
        ...
        break;
    case "powerUp2":
        winnings >= aPowerUpButsData[aPowerUpBarData[2]] && aPowerUpBarData[2] < 8 && (playSound("upgrade"),
        ...
        break;
    case "powerUp3":
        winnings >= aPowerUpButsData[aPowerUpBarData[3]] && aPowerUpBarData[3] < 8 && (playSound("upgrade"),
...
function roadCallback(a, b) {
    switch ("undefined" == typeof b && (b = null),
    a) {
    case "hitEnemyCar":
        nitroMode && (speed = maxSpeed,
        nitroMode = !1),
        speed *= .75,
...
function updateGameEvent() {
    if (!manualPause && !rotatePause && "game" == gameState) {
        var a = getDelta();
        if (road.steerX > 500 / road.roadScaleMultiplier || road.steerX < -500 / road.roadScaleMultiplier ? (speed = Math.max(speed -= 250 * a, 200),
        4 >= startTimer && (levelScore -= Math.round(25 * a)),
        offRoad || (playSound("offRoad"),
        offRoad = !0)) : (offRoad = !1,
        nitroMode ? (flexMaxSpeed = nitroSpeed - Math.abs(steerX) / 4,
        speed += a * ((flexMaxSpeed - speed) / 3),
        nitroTimer += a,
        nitroTimer > nitroLength && (nitroMode = !1,
        playSound("nitroEnd"))) : (flexMaxSpeed = maxSpeed - Math.abs(steerX) / (3 + turnRate),
        speed += a * ((flexMaxSpeed - speed) / accRate))),
        hud.speed = Math.round(speed / 4),
        raceProgress += speed * a,
        leadProgress += enemySpeed * a,
        hud.raceProgress = raceProgress / raceLength,
        carReleaseDelay += speed * a,
        raceProgress > raceLength ? (road.bridgeType = 5,
        road.bridgeRow = 0,
        hud.raceProgress = 1) : raceProgress > bridgeDistanceTarg && (road.bridgeType = 4,
        road.bridgeRow = 0,
        bridgeDistanceTarg += raceLength / 4),
        carReleasedNum > 0 && road.freeToAddCar() && raceProgress > leadProgress - carReleasedNum / 19 * .9 * leadProgress && (road.addEnemyCar(),
        carReleasedNum--,
        overtakenInc = 1,
        carReleaseDelay = 0),
        19 > racePos && enemySpeed > speed ? (speedDifferencial += enemySpeed - speed,
        speedDifferencial > 5e3 * overtakenInc && (overtakenInc++,
        road.addEnemyCar(!1),
        4 >= startTimer && (racePos++,
        playSound("undertake")),
        speedDifferencial = 0)) : speedDifferencial = 0,
        tweenScaleTimer += a,
        tweenScaleTimer > .5 && (tweenScaleTimer = 0,
        ...
        steerX += (targSteerX - steerX) * turnRate * a,
        !justSkid && (steerX > 525 || -525 > steerX) ? (playSound("skid" + Math.ceil(3 * Math.random())),
        justSkid = !0) : steerX > -100 && 100 > steerX && (justSkid = !1),
        road.update(speed, steerX, curveAmount, hillAmount, a),
        road.render(ctx),
        userCar.update(speed, steerX, curveAmount, hillAmount, rightSteerSimple + leftSteerSimple, a),
        userCar.render(ctx),
        hud.render(ctx),
        firstPlay) {
            ...
            startTimer += a,
            startTimer > 4 && (firstPlay = !1,
            playSound("start2"),
            startTimer = 4)
        } else if (2 > startTimer) {
            var b = assetLib.getData("uiElements");
            0 == startStage ? (playSound("start1"),
            startStage = 1) : 1 == startStage && 2 * startTimer > 1 ? (playSound("start1"),
            startStage = 2) : 2 == startStage && 2 * startTimer > 2 ? (playSound("start1"),
            startStage = 3) : 3 == startStage && 2 * startTimer > 3 && (playSound("start2"),
            startStage = 4);
            ...
            startTimer += a,
            endSoundPlayed || (playSound("raceEnd"),
            endSoundPlayed = !0),
...
function resizeCanvas() {
    ...
    div.style.marginLeft = "0px") : (rotatePause && rotatePauseOff(),
    a / canvas.width < b / canvas.height ? (canvas.style.width = a + "px",
    canvas.style.height = a / canvas.width * canvas.height + "px",
    canvasX = 0,
    canvasY = (b - a / canvas.width * canvas.height) / 2,
    canvasScaleX = canvasScaleY = canvas.width / a,
    div.style.marginTop = canvasY + "px",
    div.style.marginLeft = canvasX + "px") : (canvas.style.width = b / canvas.height * canvas.width + "px",
    canvas.style.height = b + "px",
...
!function(a) {
    var b = function() {
        function a(a, b, c, d, e, f) {
            ...
            ctx.lineTo(this.topLeftX + d / 4, this.topLeftY + 0),
            ctx.lineTo(this.topLeftX + d / 4, this.topLeftY + 20),
        ...
        a.prototype.getSpriteSize = function(a) {
            for (var b = new Array, c = "", d = "", e = 0, f = a.lastIndexOf("."), g = !0; g; )
                f--,
                0 == e && this.isNumber(a.charAt(f)) ? c = a.charAt(f) + c : 0 == e && c.length > 0 && "x" == a.charAt(f) ? (f--,
                e = 1,
                d = a.charAt(f) + d) : 1 == e && this.isNumber(a.charAt(f)) ? d = a.charAt(f) + d : 1 == e && d.length > 0 && "_" == a.charAt(f) ? (g = !1,
                b = [parseInt(d), parseInt(c)]) : (g = !1,
                b = [0, 0]);
            return b
        }
        ,
        ...
        a.prototype.render = function(a) {
            ...
            null != this.animId) {
                ...
                if ("once" == this.animType && c > this.maxIdx) {
                    if (this.fixedFrame = this.oAnims[this.animId][b - 1],
                    this.animId = null,
                    null != this.animEndedFunc)
                        return this.animEndedFunc(),
                        a.restore(),
                        void 0;
    ...
    var b = function() {
            ...
            if (!this.pauseIsOn) {
                ...
                for (var f = 0; f < this.aHitAreas.length; f++)
                    if (this.aHitAreas[f].rect && b > this.aHitAreas[f].area[0] && c > this.aHitAreas[f].area[1] && b < this.aHitAreas[f].area[2] && c < this.aHitAreas[f].area[3]) {
                        ...
                        this.aHitAreas[f].oData.isDown || (this.aHitAreas[f].oData.isDown = !0,
                        this.aHitAreas[f].oData.x = b,
                        this.aHitAreas[f].oData.y = c,
                ...
                for (var e = 0; e < this.aHitAreas.length; e++)
                    if (this.aHitAreas[e].rect && b > this.aHitAreas[e].area[0] && c > this.aHitAreas[e].area[1] && b < this.aHitAreas[e].area[2] && c < this.aHitAreas[e].area[3]) {
                        for (var f = 0; f < this.aHitAreas[e].aTouchIdentifiers.length; f++)
                            this.aHitAreas[e].aTouchIdentifiers[f] == d && (this.aHitAreas[e].aTouchIdentifiers.splice(f, 1),
                            f -= 1);
                        0 == this.aHitAreas[e].aTouchIdentifiers.length && (this.aHitAreas[e].oData.isDown = !1,
                        this.aHitAreas[e].oData.multiTouch && this.aHitAreas[e].callback(this.aHitAreas[e].id, this.aHitAreas[e].oData));
                        break
                    }
            }
        }
                ...
                for (var f = 0; f < this.aHitAreas.length; f++)
                    if (this.aHitAreas[f].rect)
                        if (b > this.aHitAreas[f].area[0] && c > this.aHitAreas[f].area[1] && b < this.aHitAreas[f].area[2] && c < this.aHitAreas[f].area[3])
                            ...
                            this.aHitAreas[f].oData.y = c,
                            this.aHitAreas[f].aTouchIdentifiers.push(d),
                            this.aHitAreas[f].oData.multiTouch && this.aHitAreas[f].callback(this.aHitAreas[f].id, this.aHitAreas[f].oData)),
                            this.aHitAreas[f].oData.isDraggable && (this.aHitAreas[f].oData.isBeingDragged = !0,
                            this.aHitAreas[f].oData.x = b,
                            this.aHitAreas[f].oData.y = c,
                            this.aHitAreas[f].callback(this.aHitAreas[f].id, this.aHitAreas[f].oData),
                            this.aHitAreas[f].oData.isBeingDragged = !1);
                            ...
                            for (var g = 0; g < this.aHitAreas[f].aTouchIdentifiers.length; g++)
                                this.aHitAreas[f].aTouchIdentifiers[g] == d && (this.aHitAreas[f].aTouchIdentifiers.splice(g, 1),
                                g -= 1);
                            0 == this.aHitAreas[f].aTouchIdentifiers.length && (this.aHitAreas[f].oData.hasLeft = !0,
                            this.aHitAreas[f].oData.isBeingDragged || (this.aHitAreas[f].oData.isDown = !1),
                            this.aHitAreas[f].oData.multiTouch && this.aHitAreas[f].callback(this.aHitAreas[f].id, this.aHitAreas[f].oData))
                        }
            }
        }
        ...
        a.prototype.keyDown = function(a) {
            for (var b = 0; b < this.aKeys.length; b++)
                a.keyCode == this.aKeys[b].keyCode && (this.aKeys[b].oData.isDown = !0,
                this.aKeys[b].callback(this.aKeys[b].id, this.aKeys[b].oData))
        }
        ,
        a.prototype.keyUp = function(a) {
            for (var b = 0; b < this.aKeys.length; b++)
                a.keyCode == this.aKeys[b].keyCode && (this.aKeys[b].oData.isDown = !1,
                this.aKeys[b].callback(this.aKeys[b].id, this.aKeys[b].oData))
        }
        ,
        ...
        a.prototype.removeKey = function(a) {
            for (var b = 0; b < this.aKeys.length; b++)
                this.aKeys[b].id == a && (this.aKeys.splice(b, 1),
                b -= 1)
        }
        ,
        ...
        a.prototype.removeHitArea = function(a) {
            for (var b = 0; b < this.aHitAreas.length; b++)
                this.aHitAreas[b].id == a && (this.aHitAreas.splice(b, 1),
                b -= 1)
        }
        ,
        a
...
!function(a) {
    var b = function() {
        ...
        a.prototype.renderScroll = function(a) {
            ...
            for (var c = 0; b > c; c++)
                a.drawImage(this.oImgData.img, c * (this.canvasWidth / b), 0, this.canvasWidth / b, this.canvasHeight, c * (this.canvasWidth / b), 5 * Math.sin(this.incY + c / 20), this.canvasWidth / b, this.canvasHeight)
        }
        ,
        a.prototype.render = function(a) {
            a.drawImage(this.oImgData.img, 0, 0)
    ...
    var b = function() {
        ...
        a.prototype.render = function(a, b) {
            ...
            case "levelComplete":
                ...
                for (var f = this.oScoreData.winnings, g = 0; g < f.toString().length; g++) {
                ...
                for (var f = this.oScoreData.levelScore, g = 0; g < f.toString().length; g++) {
                    c = parseFloat(f.toString().charAt(g));
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width
                      , e = Math.floor(c / (this.oNumbersImgData.img.width / this.oNumbersImgData.oData.spriteWidth)) * this.oNumbersImgData.oData.spriteHeight;
                ...
                for (var f = this.oScoreData.totalScore, g = 0; g < f.toString().length; g++) {
                    c = parseFloat(f.toString().charAt(g));
                    var d = c * this.oNumbersImgData.oData.spriteWidth % this.oNumbersImgData.img.width
                      , e = Math.floor(c / (this.oNumbersImgData.img.width / this.oNumbersImgData.oData.spriteWidth)) * this.oNumbersImgData.oData.spriteHeight;
        ...
        a.prototype.addButs = function(a) {
            for (var b = 0; b < this.aButs.length; b++) {
                var c = this.posY
                  , d = 0;
                this.aButs[b].noFloat ? (d = -this.posY,
                c = 0) : d = 3 * Math.sin(this.incY + 45 * b);
            ...
            if ("map" == this.panelType) {
                for (var i, b = 0; b < aMapPointData.length; b++)
                    if (2 == saveDataHandler.aLevelStore[3 * b]) {
...
!function(a) {
    var b = function() {
        ...
        b.prototype.addScenery = function() {
            ...
            switch (this.sceneryDir++,
            this.bridgeRow < 20 && 0 == this.sceneryDir % 10 && (this.bridgeRow++,
            b = this.bridgeType),
            b) {
            case 0:
        ...
        b.prototype.addEnemyCar = function(b) {
            ...
            b || (c.rowNum = this.segNum - 1,
            c.driftInc = this.steerX < 0 ? 30 : -30,
            c.driftPower = 1e3),
            this.carSpace = 100 * Math.random() + 50,
        ...
        b.prototype.update = function(a, b, c, d, e) {
            ...
            this.sceneryInc > 5 && (this.sceneryInc = 0,
            this.addScenery()),
            nitroMode ? this.nitroInc = 0 : this.nitroInc += this.speed * this.delta,
            this.nitroInc > 5e3 && (this.nitroInc = 0,
            this.addNitro()),
            this.steerX > 600 / this.roadScaleMultiplier ? (this.steerX = 600 / this.roadScaleMultiplier,
            nitroMode = !1) : this.steerX < -600 / this.roadScaleMultiplier && (this.steerX = -600 / this.roadScaleMultiplier,
            nitroMode = !1),
            this.scrollY -= a * e,
        ...
        b.prototype.freeToAddCar = function() {
            for (var a = this.aEnemyCars.length - 1; a >= 0; a--)
                if (this.aEnemyCars[a].rowNum < this.carSpace)
                    return !1;
            return !0
        }
        ,
        b.prototype.render = function(a) {
            ...
            for (var b = 500 / this.segNum, c = 0, d = 0; d < this.segNum; d++)
                this.tempInc = d,
                ...
                c > 1 && (a.drawImage(this.oRoadImgData.img, 0, this.rowId * b + this.scrollY, this.oRoadImgData.oData.spriteWidth, b, this.aRowData[this.rowId].x, this.aRowData[this.rowId].y, this.aRowData[this.rowId].scale, c + 1),
                c = 0);
            ...
            for (var d = this.aEnemyCars.length - 1; d >= 0; d--)
                this.aEnemyCars[d].x = this.aRowData[Math.floor(this.aEnemyCars[d].rowNum)].x + this.aRowData[Math.floor(this.aEnemyCars[d].rowNum)].scale * this.aEnemyCars[d].sideMultiplier,
                ...
                this.aEnemyCars[d].y > 345 + 20 * this.hillAmount && this.aEnemyCars[d].x < this.canvasWidth / 2 + 80 && this.aEnemyCars[d].x > this.canvasWidth / 2 - 80 ? this.aEnemyCars[d].canHit && (this.aEnemyCars[d].canHit = !1,
                this.aEnemyCars[d].y > 380 + 20 * this.hillAmount && (this.aEnemyCars[d].speed = 0),
                this.callback("hitEnemyCar", {
                    bounceX: 1 / 75 * (this.aEnemyCars[d].x - this.canvasWidth / 2)
                })) : this.aEnemyCars[d].canHit = !0,
                this.aEnemyCars[d].rowNum >= this.segNum ? (this.aEnemyCars.splice(d, 1),
                4 >= startTimer && (racePos--,
                playSound("overtake"))) : this.aEnemyCars[d].rowNum < 0 && (this.aEnemyCars.splice(d, 1),
                carReleasedNum++);
            this.nitro && (this.nitro.x = this.aRowData[Math.floor(this.nitro.rowNum)].x + this.aRowData[Math.floor(this.nitro.rowNum)].scale * this.nitro.sideMultiplier,
            ...
            this.nitro.render(a),
            this.nitro.rowNum += .6 * (this.speed - this.nitro.speed) * this.delta,
            this.nitro.y > 345 + 20 * this.hillAmount && this.nitro.x < this.canvasWidth / 2 + 65 && this.nitro.x > this.canvasWidth / 2 - 65 && (this.nitro.rowNum = -1,
            this.callback("hitNitro")),
            (this.nitro.rowNum >= this.segNum || this.nitro.rowNum < 0) && (this.nitro = null));
            for (var d = this.aScenery.length - 1; d >= 0; d--)
                this.aScenery[d].x = this.aRowData[Math.floor(this.aScenery[d].rowNum)].x + this.aRowData[Math.floor(this.aScenery[d].rowNum)].scale * this.aScenery[d].sideMultiplier,
...
!function(a) {
    var b = function() {
        ...
        return a.prototype.clearData = function() {
            ...
            for (var a = 0; a < this.totalLevels - 1; a++)
                this.aLevelStore.push(0),
                this.aLevelStore.push(20),
                this.aLevelStore.push(0);
            for (var a = 0; 4 > a; a++)
...
isIE10 || "undefined" == typeof window.AudioContext && "undefined" == typeof window.webkitAudioContext && -1 != navigator.userAgent.indexOf("Android") ? audioType = 0 : (audioType = 1,
sound = new Howl({
