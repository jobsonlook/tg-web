...
com4j.config = {
    ...
    ForJoyH5_DefaultBigLogo: "./game/logo/default_b.png?1466980738",
    _ForJoyH5_BigLogo: "./game/logo/default_b.png?1466980738",
    ...
    ForJoyH5_DefaultSmallLogo: "./game/logo/default_s.png?1466980738",
    _ForJoyH5_SmallLogo: "./game/logo/default_s.png?1466980738",
...
com4j.moregames = {
    ...
    init: function() {
        ...
        var d = com4j.config.host + "moregames/" + com4j.config.NameID + ".json?v=" + e;
    ...
    randomInit: function() {
        ...
        while (d.length > 0) {
            ...
            var e = d[b];
            d.splice(b, 1);
            if (!com4j.config.ForJoyH5_ShowCopyright && e.hasOwnProperty("copyright") && e.copyright == true) {
                continue
            }
            var a = com4j.config.host + e.nameid + "/index.php?pubid=" + com4j.config.ForJoyH5_pubid;
            f += "<a href='" + a + "'>";
            ...
            f += "</span>";
            f += "</p>";
            f += "<span class='new_ui_sprite new_ui_button-next new_ui_icon-right-big-button'></span>";
            f += "</div>";
            f += "</a>"
        }
...
com4j.ad = {
    ...
    get adTagUrl() {
        var b = "game=" + com4j.config.ForJoy_gameid + "&pubid=" + com4j.config.ForJoyH5_pubid + "&video=" + com4j.ad.adType + "&uid=" + com4j.config.ForJoyH5_pubid + "_" + com4j.config.ForJoy_gameid + "&mid=" + com4j.config.ForJoyH5_mid;
        b = encodeURIComponent(b);
        var a = "https://pubads.g.doubleclick.net/gampad/ads?sz=640x480&iu=/324879675/pregame_h5.4j.com_parnter&cust_params=" + b + "&impl=s&gdfp_req=1&env=vp&output=vast&unviewed_position_start=1";
        return a
    },
    check: function() {
    ...
    _init: function() {
        ...
        var a = new google.ima.AdDisplayContainer(com4j.ad._imaContainer, com4j.ad._videoContent);
        a.initialize();
    ...
    init: function() {
        ...
        if ((com4j.config.ForJoyH5_ShowPreGameAd || a.indexOf(com4j.config.host) == 0) && com4j.config.ForJoyH5_stats) {
            com4j.ad.show()
        } else {
            com4j.ad.onAdClose()
        }
    ...
    getShowable: function(d) {
        if (com4j.ad._lastInGameAdTime === -1 || d) {
            return true
        }
        var b = (new Date()).getTime();
...
com4j.game = {
    ...
    preInit: function() {
        ...
        MGDelegate.addEventListener("AD_CLOSE_PRE", function(c) {
        ...
        if ($("script[data-load]").data("load")) {
            ...
            for (var a = 0; a < $vers.length - 1; a++) {
                com4j.loadScript($vers[a], function() {
                    b++;
                    if (b == $vers.length - 1) {
                        if (com4j.game.inited == true) {
                            com4j.game.init()
    ...
    onLoadComplete: function(d) {
        var c = 100;
        com4j.loading.setProgress(c);
        var b = new Date().getTime() - com4j.ga._loadStart;
        com4j.ga.sendTiming("LOAD_TIME", b);
        var a = function() {
            $("#preloaded").animate({
                opacity: 1,
                marginTop: 0,
...
com4j.ga = {
    ...
    send: function() {
        var a = ["send"].concat(arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments));
    ...
    sendEvent: function(b, a, d, e, f) {
        var c = ["event", com4j.config.NameID].concat(arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments));
        this.send.apply(window, c)
    },
    sendTiming: function(a, e, d, c, f) {
        var b = ["timing", com4j.config.NameID].concat(arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments));
        this.send.apply(window, b)
    },
    sendSocial: function(c, b, d, e) {
        var a = ["social", com4j.config.NameID].concat(arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments));
        this.send.apply(window, a)
    },
    sendException: function(c, b) {
        var a = ["exception"].concat(arguments.length === 1 ? [arguments[0]] : Array.apply(null, arguments));
        this.send.apply(window, a)
    }
};
com4j.share = {
    ...
    _toQQ: function(e, c, b, d) {
        // var f = com4j.share._getTopHref() || com4j.config.host + e + "/index.php?pubid=" + d;
        // f = encodeURIComponent(f);
        // var a = "https://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?url=" + f + "&title=" + c + " &pics=" + com4j.config.host + "thumb/" + e + ".jpg&summary=" + b;
        // window.open(a)
    },
    _toWeibo: function(e, c, b, d) {
        // var f = com4j.share._getTopHref() || com4j.config.host + e + "/index.php?pubid=" + d;
        // f = encodeURIComponent(f);
        // var a = "https://service.weibo.com/share/share.php?url=" + f + "&title=" + b + "&searchPic=true&pic=" + com4j.config.host + "thumb/" + e + ".jpg";
        // window.open(a)
    },
    _toFacebook: function(e, c, b, d) {
        // var f = com4j.share._getTopHref() || com4j.config.host + e + "/index.php?pubid=" + d;
        // f = encodeURIComponent(f);
        // var a = "https://www.facebook.com/dialog/feed?app_id= 1692226401062334&display=popup&caption=" + b + "&link=" + f + "&redirect_uri=" + f + "&picture=" + com4j.config.host + "thumb/" + e + ".jpg";
        // window.open(a)
    },
...
com4j.modal = {
    ...
    init: function() {
        ...
        $("#modal").bind("touchstart touchend touchmove mousedown mouseup mousemove", a)
    },
    show: function(a) {
...
com4j.init = function() {
    var h = "color:yellow;background:green;";
    var a = "%c%s";
    console.log(a, h, "Com4jAPI VERSION " + this.VERSION);
    ...
    if (window.location.search != "") {
        ...
        e = f.pubid || "default";
        b = f.v || "";
        if (f.hasOwnProperty("debug") || f.hasOwnProperty("_ijt")) {
            com4j.config.debug = true
        }
        if (f.hasOwnProperty("dev") && com4j.config.debug) {
            com4j.config.devMode = true
        }
        com4j.config.parameters = f
    }
    var d = true;
    ...
    //     d = false;
        com4j.config.debug = true
    // }
    com4j.config.ForJoyH5_pubid = e;
    com4j.config.v = b;
    var c =  "./game/setting/" + e + ".json?" + b;
    $.get(c, function(n, j, o) {
        if (j == "success") {
            for (var l in n) {
                var k = "ForJoyH5_" + l;
                if (com4j.config.hasOwnProperty(k)) {
                    var m = n[l];
                    if (typeof m == "string") {
        ...
        if (com4j.config.ForJoyH5_ShowAd && !com4j.ad.check()) {
            ...
            // $.get("//freegeoip.net/json/", function(n) {
            //     var m = n.country_code + "/" + n.country_name + "/" + n.region_code + "/" + n.region_name + "/" + n.city;
            //     com4j.ga.sendEvent("ADVERTISING", "BLOCKED", m)
            // });
            return
        }
        com4j.credits.init();
        ...
        if (d) {
            var i = "./ajax_addgameplay.php?pubid=" + com4j.config.ForJoyH5_pubid + "&mid=" + com4j.config.ForJoyH5_mid + "&game=" + com4j.config.ForJoy_gameid + "";
            $.get(i)
        }
        $(document.body).css("background", com4j.config.ForJoyH5_BgColor);
        ...
        if (com4j.initCallbackObj) {
            var l = com4j.initCallbackObj.callback;
            var j = com4j.initCallbackObj.context;
            var k = com4j.initCallbackObj.params;
            l.apply(j, k)
        }
    })
};
