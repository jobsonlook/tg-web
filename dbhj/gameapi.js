...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.init = function() {
		...
		var sdkLoadedCallback = function() {
			M.Ad4GameCounter = 0;
		...
		switch (self.config.ads.provider) {
			case 'adsense':
				faZepto.getScript("./ima3.js", 
					sdkLoadedCallback, 
					function() {
						self.tracking.trackEvent("AdBlocker event", "AdSense", famobi_gameID);
					});
				break;
			case 'dfp':
				M.googletag = M.googletag || {};
				M.googletag.cmd = M.googletag.cmd || [];
				(function() {
				...
				M.googletag.cmd.push(function() {
					alert("hello");
					M.googletag.defineSlot('/37336410/TestCode', [[250, 250], [300, 250], [336, 280]], 'div-gpt-ad-1424882062888-0').addService(M.googletag.pubads());
					M.googletag.pubads().enableSingleRequest();
					M.googletag.enableServices();
					sdkLoadedCallback();
				});
				break;
			case 'ad4game':
				faZepto.getScript("./4game_sdk.js", 
					sdkLoadedCallback, 
					function() {
						self.tracking.trackEvent("AdBlocker event", "Ad4Game", famobi_gameID);
					});
				break;
			case 'leadbolt':
			case 'housead':
	...
	module.prototype.showAdsense = function(callback, force){
		...
		M.adContainerElement.id = 'fg-ad-container';
		// M.adContainerElement.style.position = 'absolute';
		M.adContainerElement.style.overflow = 'hidden';
		M.adContainerElement.style.width = "300px";
		M.adContainerElement.style.height = "250px";
		...
		try {
			...
			M.showAdModal({"returnCallback": callback, "width": parseInt(M.adContainerElement.style.width), "height": parseInt(M.adContainerElement.style.height)});
		} catch (reason) {
			M.showAdModal({"returnCallback": callback, "width": parseInt(M.adContainerElement.style.width), "height": parseInt(M.adContainerElement.style.height)});
		...
		module.prototype.onAdsManagerLoaded = function(adsManagerLoadedEvent){
			...
			M.adsManager.init(parseInt(M.adContainerElement.style.width), parseInt(M.adContainerElement.style.height), google.ima.ViewMode.NORMAL);
			M.adsManager.start();
		...
		module.prototype.onAdEvent = function(adEvent){
			...
			switch (adEvent.type) {
				case google.ima.AdEvent.Type.LOADED:
					...
					if (!ad.isLinear()) {
						if (ad.getHeight()) {
							self.modal.setDimensions(M.adContainerElement.style.width, ad.getHeight()-4);
						} else {
							//self.modal.setDimensions(ad.getWidth(), ad.getHeight()-4);
						}
					}
		...
		google.ima.AdsRenderingSettings.AUTO_SCALE = false;
		google.ima.AdsRenderingSettings.autoAlign = false;
		google.ima.AdsRenderingSettings.useStyledNonLinearAds = false;
		...
		*/
		var publisherId="ca-games-pub-8700401253704627"; // AdSense for Games
		// var publisherId="ca-games-pub-2504503821144226"; // Ad Exchange for Games
		...
		adsRequest.nonLinearAdSlotWidth = faZepto(M.adContainerElement).width();
		adsRequest.nonLinearAdSlotHeight = faZepto(M.adContainerElement).height();
	...
	module.prototype.initAdModal = function(options){
		...
		if (!M.isAdvertisingActive(opts) || M.delayNextAdModal(opts.force)) {
	...
	module.prototype.showDFP = function(callback, force){
		var opts = {
			mode: "ad",
			width: 300,
			height: 250,
		...
		self.log("show dfp");// Add other options here
		...
		// show banner
		self.modal.create(opts);
		self.modal.setContent(faZepto("<div id='div-gpt-ad-1424882062888-0'></div>").get(0));
		self.modal.setHeader(self.translate("api.ad_modal_header2"));
		self.modal.setCloseCallback(opts.returnCallback);
		//self.modal.updateCloseBtn(true);

		M.googletag.cmd.push(function() { M.googletag.display('div-gpt-ad-1424882062888-0'); });

		return self;
		
	};

	module.prototype.showInterstitial = function(callback, force){
		var opts = {
			mode: "",
			min_s_between: (+self.config.ads.min_s_between > 0) ? +self.config.ads.min_s_between : 30000,
			iframeAllowed: true,
		...
		self.log("show interstitial");// Add other options here
		...
		// show banner
		ad4game_html5_sdk.setBrand(self.getMoreGamesButtonImage(), self.translate("more_games_url"), "Find more games on");
		ad4game_html5_sdk.setWaitText(self.translate("api.ad_modal_header2"));
		ad4game_html5_sdk.setAdBanner("46865");

		self.tracking.trackEvent('Ad event', 'Ad4Game Interstitial', famobi_gameID);

		ad4game_html5_sdk.isMobile(true);

		if (M.Ad4GameCounter < 1)
			ad4game_html5_sdk.init();
		else
			ad4game_html5_sdk.interLevelShowAd(5, true);
	...
	module.prototype.showHouseAd = function(callback, force){
		...
		self.log("show house ad");// Add other options here
		...
		// show banner
		var url = '/ads/housead/' + window.famobi_gameID + '/' + window.famobi.config.aid;
	...
	module.prototype.showAppWall = function(callback, force){
		var opts = {
			mode: "ad",
			showCloseBtn: true,
			min_s_between: (+self.config.ads.min_s_between > 0) ? +self.config.ads.min_s_between : 30000,
		...
		if (!detection.is.ios && !detection.is.android) {
			self.log("skipped ad (OS not supported)");
			opts.returnCallback();
			return false;
		}
		...
		// show iframe
		var url = '/ads/appwall/' + window.famobi_gameID + '/' + window.famobi.config.aid;
		self.iframe.show(url, opts);
		self.modal.setHeader(self.translate("api.ad_modal_header2"));
		self.modal.setCloseCallback(opts.returnCallback);
		//self.modal.updateCloseBtn(true);
...
fg_api.prototype.orientationModule = function() {
	...
	orientationPrototype.init = function(){
		if (typeof self.config.gameParams.orientation != 'undefined' && 
			...
			fgLandscapeImage.setAttribute("src", "./gameapi/v1/images/RotateToLandscape.png");
			...
			fgPortraitImage.setAttribute("src", "./gameapi/v1/images/RotateToPortrait.png");
...
fg_api.prototype.paymentModule = function() {
	...
	function setUpPayment() {
		...
		self.iframe
			.subscribePostMessageListener('payment', 'displayWidget', function(data){
				...
				if (self.newWindow && 0) {
					self.newWindow.location.href = iframe.attr('src');
				} else {
					self.modal.create({
						showCloseBtn: false,
						transparent: true
			...
			.subscribePostMessageListener('payment', 'purchase', function(data){
				...
				if (data.action == 'listsilent') {
					if (callbacks.listsilent) {
						callbacks.listsilent(data);
						self.tracking.trackEvent("Payment event", "Payment List silent");
					}
					return;
				} else if (data.action === 'success') {
					if (callbacks.success) {
						callbacks.success(data);
						self.tracking.trackEvent("Payment event", "Payment success");
						return;
	...
	function onPaymentSuccess(provider) {
	...
	function openWidget(itemID, callback, provider) {
		provider = provider || getProvider();
		...
		//if (!detection.is.pc) {
		//	// open new window for mobile browsers
		//	self.newWindow = window.open();
		//}
		return true;
	}

	paymentProt.showPayment = function(itemID, callback) {
		openWidget(itemID, callback);
	};

	paymentProt.getPurchase = function(cb) {
		provider = provider || getProvider();
		var action;
	...
	paymentProt.consumed = function(item, amount) {
		provider = provider || getProvider();
...
fg_api.prototype.init = function() {
	...
	newStylesheet.setAttribute("href", "./play.css");
